...
`torch.cuda.is_bf16_supported()` returns `True` on a Tesla T4. It's lying to you.
...
Anyone fine-tuning on:
- Kaggle free-tier T4 notebooks
- Google Colab T4 instances
- AWS g4dn instances (T4)
- Any Turing-architecture GPU (T4, RTX 2080, Quadro RTX)

If your training code uses `is_bf16_supported()` to pick the AMP dtype, and you're running on any of these GPUs, you're leaving 4x performance on the table.

## The lesson

"Supported" and "accelerated" are different things. The T4 supports bf16 the same way your laptop supports running a marathon — technically possible, not recommended, and much slower than the alternative.
